...
const popupStyle = (color) => ({
  ...
  borderRadius: '8px', minWidth: '190px', fontFamily: 'sans-serif', fontSize: '12px'
})
...
const filteredCrimes = (() => {
  ...
  if (filterType !== 'all') {
    result = result.filter(c => c.crimeType?.toLowerCase() === filterType)
  }

  return result
})()

  const crimeFreq  = filteredCrimes.reduce((a, c) => {
    const t = c.crimeType?.toLowerCase() ?? '?'
    a[t] = (a[t] || 0) + 1
    return a
  }, {})
  ...
  const topRiskScore = topRiskZone
    ? Math.min(100, Math.round((topRiskZone.crimeCount / Math.max(...zones.map(z => z.crimeCount))) * 100))
    : 0
  ...
  const CrimePopup = ({ crime }) => {
    ...
    return (
      <div style={popupStyle(color)}>
        ...
        <div style={{ marginBottom: '3px' }}>{crime.description}</div>
        <div style={{ color: '#71717a', fontSize: '11px', marginBottom: '2px' }}>
          By: {crime.reportedBy}
        </div>
        <div style={{ color: '#71717a', fontSize: '11px', marginBottom: '6px' }}>
          {crime.occurredAt?.split('T')[0]}
        </div>
        <span style={{
  ...
  return (
    <div className="relative w-full font-sans" style={{ height: '100vh' }}>
      ...
      {/* ── Top Tab Bar ── */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-1 bg-[#0f0f0f]/90 backdrop-blur border border-zinc-700 rounded-full px-2 py-1.5">
        {tabs.map(tab => (
          <button key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 py-1.5 rounded-full text-xs font-medium tracking-wide transition-all cursor-pointer capitalize
      ...
      {/* ── Left Panel ── */}
      <div className="absolute top-16 left-3 z-[1000] w-44 space-y-2">
      ...
      {/* ── Right Panel ── */}
      <div className="absolute top-16 right-3 z-[1000] w-44">
        <div className="bg-[#0f0f0f]/90 backdrop-blur border border-zinc-700 rounded-lg p-3 space-y-3">

          {/* Incident count */}
          <div>
            <div className="text-2xl font-semibold text-white tabular-nums">{filteredCrimes.length}</div>
          ...
          {/* Hotspot summary */}
          <div>
            <div className="text-xs text-zinc-500 mb-1.5 font-medium">Hotspots</div>
          ...
          {/* Case status */}
          <div>
            <div className="text-xs text-zinc-500 mb-1.5 font-medium">Case Status</div>
          ...
          {/* Prediction zone breakdown */}
          {activeTab === 'prediction' && (
            <>
              <div className="h-px bg-zinc-800" />
              <div>
                <div className="text-xs text-zinc-500 mb-1.5 font-medium">Prediction Zones</div>
                {[
                  { t: 'CAMERA', c: '#3b82f6' },
                  { t: 'KOBAN',  c: '#ef4444' },
                  { t: 'PATROL', c: '#22c55e' },
                  ...
                  <div key={z.t} className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-zinc-300 tabular-nums">
                      {zones.filter(zn => zn.zoneType === z.t).length}
                    </span>
                  </div>
                ))}
              </div>
          ...
          {/* Crime type legend for incidents */}
          {(activeTab === 'incidents' || activeTab === 'patrol') && (
            <>
              <div className="h-px bg-zinc-800" />
              <div>
                <div className="text-xs text-zinc-500 mb-1.5 font-medium">Crime Types</div>
                <div className="space-y-1">
                  {Object.entries(CRIME_COLORS).slice(0, 6).map(([type, color]) => (
                    <div key={type} className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: color }} />
                      <span className="text-xs text-zinc-600 capitalize truncate">{type}</span>
                    </div>
                  ))}
                  {Object.entries(CRIME_COLORS).length > 6 && (
                    <div className="text-xs text-zinc-700">+{Object.entries(CRIME_COLORS).length - 6} more</div>
                  )}
                </div>
              </div>
            </>
          ...
          {/* Risk level legend for hotspots */}
          {activeTab === 'hotspots' && (
            <>
              <div className="h-px bg-zinc-800" />
              <div>
                <div className="text-xs text-zinc-500 mb-1.5 font-medium">Risk Level</div>
                <div className="space-y-1">
                  {[
                    ['#ef4444', 'Critical ≥75'],
                    ['#f97316', 'High 55–74'],
      ...
      {selectedZone && (
        <div className="absolute top-16 right-52 z-[1000] w-60 bg-[#0f0f0f]/95 backdrop-blur border border-zinc-700 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
            ...
            <button
              ...
              className="text-zinc-600 hover:text-zinc-300 text-lg leading-none cursor-pointer">
              ×
            </button>
          </div>
          <div className="p-4 space-y-3">
            ...
            <div>
              <div className="text-xs text-zinc-500 mb-1.5">Risk Assessment</div>
      ...
      <MapContainer
          ...
          return (
            <Circle
              ...
              pathOptions={{
                ...
                fillColor: color,
                fillOpacity: 0.12,
                weight: score >= 75 ? 2.5 : 1.5,
                dashArray: score < 55 ? '5 4' : undefined,
              }}
              eventHandlers={{ click: () => setSelectedZone(zone) }}>
              <Popup>
                <div style={popupStyle(color)}>
                  <div style={{ color, fontWeight: 600, fontSize: '13px', marginBottom: '6px' }}>
                    {getRiskLabel(score)} — {zone.zoneType}
                  </div>
                  <div style={{ marginBottom: '3px' }}>Incidents: {zone.crimeCount}</div>
                  <div style={{ marginBottom: '3px', textTransform: 'capitalize' }}>
                    Dominant: {zone.dominantCrime}
                  </div>
        ...
        {activeTab === 'prediction' && zones.map((zone, i) => {
          ...
          return (
            <Circle
              ...
              <Popup>
                <div style={popupStyle(colors.stroke)}>
                  <div style={{ color: colors.stroke, fontWeight: 600, fontSize: '13px', marginBottom: '6px' }}>
                    {label}
                  </div>
                  ...
                  <div style={{ marginBottom: '3px' }}>
                    Coverage: {Math.round(zone.radiusMeters * 1.2)}m radius
                  </div>
                  <div style={{ color: '#71717a', fontFamily: 'monospace', fontSize: '11px', marginTop: '4px' }}>
                    {zone.centerLat?.toFixed(4)}, {zone.centerLng?.toFixed(4)}
                  </div>
        ...
        {/* ── PATROL — zones with crime markers overlaid ── */}
        {activeTab === 'patrol' && (
          <>
            {zones.map((zone, i) => {
              const colors = ZONE_COLORS[zone.zoneType] ?? ZONE_COLORS.PATROL
