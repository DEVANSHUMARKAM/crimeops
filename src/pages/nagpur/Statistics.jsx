...
export default function Statistics() {
  ...
  const crimeFreq = crimes.reduce((a,c)=>{const t=c.crimeType?.toLowerCase()??'?';a[t]=(a[t]||0)+1;return a},{})
  ...
  const Label = ({k,v,s}) => (
    <div className="bg-[#141414] border border-zinc-800 rounded-lg p-4">
      ...
      <div className="text-xs font-medium text-zinc-400 mt-1">{k}</div>
      ...
      <div className="flex border-b border-zinc-800 overflow-x-auto">
        {[['overview','Overview'],['temporal','Temporal'],['incidents','Incidents']].map(([id,label])=>(
          <button key={id} onClick={()=>{setTab(id);setPage(1)}}
            className={`px-4 md:px-5 py-2.5 text-xs font-medium tracking-wide border-b-2 transition-colors cursor-pointer -mb-px whitespace-nowrap
            ...
            <div className="bg-[#141414] border border-zinc-800 rounded-lg p-5">
              ...
              <div className="flex items-center justify-center">
                <PieChart width={130} height={130}>
                  <Pie data={[{name:'Open',value:open},{name:'Closed',value:closed}]}
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={r.l} className="flex items-center gap-3">
                    <div className="text-xs text-zinc-500 w-14">{r.l}</div>
              ...
              {allZones.sort((a,b)=>b.crimeCount-a.crimeCount).slice(0,5).map((z,i)=>{
                ...
                return(
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="text-xs text-zinc-500 font-mono w-24 hidden sm:block">{z.centerLat?.toFixed(4)}°N</div>
                    <div className="flex-1 h-0.5 bg-zinc-800 rounded-full overflow-hidden">
        ...
        <div className="space-y-4">
            ...
            <div className="bg-[#141414] border border-zinc-800 rounded-lg p-5">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={trend30}>
                  <XAxis dataKey="date" tick={{fill:'#52525b',fontSize:9}} axisLine={false} tickLine={false} interval={6}/>
                  <YAxis tick={{fill:'#52525b',fontSize:10}} axisLine={false} tickLine={false}/>
                  <Tooltip {...TT}/>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#141414] border border-zinc-800 rounded-lg p-5">
              ...
              <div className="text-3xl md:text-4xl font-semibold text-white tabular-nums">{peakH.hour}:00</div>
            ...
            <div className="bg-[#141414] border border-zinc-800 rounded-lg p-5">
              ...
              <div className="text-3xl md:text-4xl font-semibold text-white">{peakD.day}</div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={exportCSV}
              className="flex items-center gap-2 px-3 py-2 bg-[#141414] border border-zinc-800 rounded text-zinc-400 text-xs font-medium hover:border-zinc-600 hover:text-zinc-200 transition-all cursor-pointer">
                ...
                <thead>
                  <tr className="border-b border-zinc-800">
                    {['Case No','Date/Time','Type','Status','Description','Reported By'].map(h=>(
                      <th key={h} className="px-3 md:px-4 py-3 text-left text-xs text-zinc-600 font-medium whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-800/50">
                  {rows.length===0?(
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-xs text-zinc-700">No results</td></tr>
                  ):rows.map(c=>(
                    <tr key={c.crimeId} className="hover:bg-zinc-900/40 transition-colors">
                      <td className="px-3 md:px-4 py-3 text-xs text-blue-500 font-mono whitespace-nowrap">{c.caseNumber??'—'}</td>
                      ...
                      <td className="px-3 md:px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-zinc-300 capitalize whitespace-nowrap">{c.crimeType}</span>
                        </div>
                      </td>
                      <td className="px-3 md:px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded whitespace-nowrap ${c.status?.toLowerCase()==='open'?'text-red-400':'text-zinc-400'}`}>
                          {c.status??'open'}
                        </span>
                      </td>
                      <td className="px-3 md:px-4 py-3 text-xs text-zinc-600 max-w-xs truncate">{c.description??'—'}</td>
                      <td className="px-3 md:px-4 py-3 text-xs text-zinc-500 whitespace-nowrap">{c.reportedBy??'—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                className="p-1.5 rounded border border-zinc-800 text-zinc-500 hover:text-white disabled:opacity-30 cursor-pointer">
                <ChevronLeft size={13}/>
              </button>
